...
# <<< added: import your loss function
...
def main():
    ...
    with torch.no_grad():
        ...
        # <<< added: compute loss if mask exists
        loss_val = None
        if mask_t is not None:
            loss_val = bce_dice(logits, mask_t.to(device)).item()
            print(f"[INFO] Loss = {loss_val:.4f}")
        else:
            print("[INFO] No mask, loss skipped")

    # ---- figure ----
    fig, axs = plt.subplots(1, 3, figsize=(16, 4))
    
    # <<< added: put loss in the suptitle
    if loss_val is not None:
        fig.suptitle(f"Sample: {img_path.name} | Loss = {loss_val:.4f}", fontsize=14)
    else:
        fig.suptitle(f"Sample: {img_path.name} | Loss: N/A", fontsize=14)
