...
def parse_args():
    p = argparse.ArgumentParser(description="Inference with layered TIFF output")
...
def load_image(path: Path):
    ...
    # Match training normalization if your model was trained with it
    img_t = T.Normalize(
...
def make_info_layer(w, h, code, d, i):
...
def main():
    ...
    for code, img_path, mask_path in pairs:
        ...
        fn_layer = make_layer(fn.numpy(), (0, 0, 255, 180))
        fp_layer = make_layer(fp.numpy(), (255, 0, 0, 180))
        tp_layer = make_layer(tp.numpy(), (0, 255, 0, 180))
        info_layer = make_info_layer(base.width, base.height, code, d, i)

        out_path = out_dir / f"{code}_inference_layers.tiff"
        base.save(
            out_path,
            save_all=True,
            append_images=[fn_layer, fp_layer, tp_layer, info_layer],
        )

        print(f"{code} | Dice={d:.4f} | IoU={i:.4f}")
